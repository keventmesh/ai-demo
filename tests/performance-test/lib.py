...
async def wait_for_reply(conn, upload_id, fake):
    if fake:
        await asyncio.sleep(0.1)
        return True, {"uploadId": "fake_" + uuid.uuid4().hex, "probability": 0.5, "x0": 0.1, "x1": 0.2, "y0": 0.3, "y1": 0.4, "created_on": "2021-01-01T00:00:00.000Z"}
...
async def disconnect_ws(conn, fake):
    ...
    try:
        ...
        logger.debug(f"Failed to disconnect from reply service: {e}")
        return False, str(e)
